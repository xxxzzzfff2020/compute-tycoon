...
export const REVIEW_CHECKPOINTS = [
  ...
  },
  {
    ...
    label: "模型研发非回退回归",
    description: "固定语音主力 Lv12 与知识蒸馏弱候选，验证研发后不降级且图鉴永久保留。",
    focus: "12→7 回归、收藏算力成长、研发回执",
  },
  {
    code: "L",
...
export function reviewCheckpointInvariantIssues(state: SaveData, id: ReviewCheckpointId): string[] {
  ...
  switch (id) {
    case "new_game":
      ...
      break;
    case "automation_unlocked":
      if (!automationUnlocked(state) || !state.automation || state.serverCount !== 0) issues.push("automation_not_just_unlocked");
      break;
    case "first_server_almost":
    ...
    case "model_research_regression": {
      ...
      if (state.modelProgress?.modelId !== "voice"
        || modelLevel(state) !== 12
        || currentCompute !== "3.8178"
        || currentIncome !== "309.2832"
      ...
      if (candidateCompute !== "2.5856" || candidateIncome !== "202.4223") {
