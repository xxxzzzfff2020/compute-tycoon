...
/** 每单位可读文本的界限 */
const CN_UNITS: Array<{ e: number; label: string }> = [
  { e: 4, label: "万" },
  { e: 8, label: "亿" },
  { e: 12, label: "兆" },
  { e: 16, label: "京" },
...
/** 顶部当前资金：阈值内保留完整千分位；超过后按兆→京→科学计数法。 */
export function formatHeaderMoney(value: Decimal | number | string): string {
  ...
  return `¥${sign}${formatBig(abs)}`;
}

/**
 * 永续终局实时资金：保留两位中文数量级小数，让持续收入在没有新按钮时仍可被看见。
...
export function formatLiveMoney(
  ...
  const sign = v.isNegative() ? "-" : "";
  const abs = v.abs();
  if (abs.lte(HEADER_MONEY_CUTOFF)) return "¥" + sign + withThousands(abs);
  ...
  if (best && expNum < 20) {
    ...
    return `¥${sign}${abs.div(unit).toFixed(digits)}${best.label}`;
  }
  ...
  const stepExp = step.gt(0) && step.lt(1)
    ...
    : 0;
  const digits = Math.max(2, Math.min(6, -stepExp));
  return `¥${sign}${abs.div(unit).toFixed(digits)}e${expNum}`;
}
